...
public class FraudeController {
    ...
    @GetMapping("/cuentas/con-ciclos")
    @Operation(
        summary = "Cuentas con ciclos activos (selector filtrado)",
        description = """
            Retorna cuentas que participan en al menos un ciclo de transferencias (lavado circular).
            Útil para alimentar selects de la UI investigativa sin cargar todas las cuentas.
            """
    )
    public ResponseEntity<List<CuentaSelectDto>> cuentasConCiclos() {
        return ResponseEntity.ok(fraudeService.cuentasConCiclosActivos());
    }

    @GetMapping("/cuentas/origenes-path")
    @Operation(
        summary = "Cuentas origen sospechosas (selector filtrado)",
        description = """
            Retorna cuentas que transfieren hacia las mulas del patrón de smurfing.
            Útil como lista de opciones de origen para el análisis de shortest path.
            """
    )
    public ResponseEntity<List<CuentaSelectDto>> cuentasOrigenPath() {
        return ResponseEntity.ok(fraudeService.cuentasOrigenSospechosas());
    }

    @GetMapping("/cuentas/destinos-path")
    @Operation(
        summary = "Cuentas destino hub (selector filtrado)",
        description = """
            Retorna nodos concentradores finales del flujo de fraude (receptores de mulas).
            Útil como lista de opciones de destino para el análisis de shortest path.
            """
    )
    public ResponseEntity<List<CuentaSelectDto>> cuentasDestinoPath() {
        return ResponseEntity.ok(fraudeService.cuentasDestinoHubs());
    }
