...
public interface PersonaRepository extends Neo4jRepository<Persona, String> {
    ...
    @Query("""
            ...
            RETURN p.id              AS personaId,
                   ...
                   toInteger(p.nivelRiesgo)           AS nivelRiesgo,
                   p.verificada      AS verificada,
                   toInteger(personasVinculadas)      AS personasVinculadas,
                   toInteger(dispositivosCompartidos) AS dispositivosCompartidos,
                   toInteger(totalCuentas)            AS totalCuentas,
                   idsVinculados     AS personasRelacionadasIds
            ORDER BY personasVinculadas DESC, totalCuentas DESC
            LIMIT $limite
            """)
    List<IdentidadSinteticaDto> findIndicadoresIdentidadSintetica(
